...
from arqick_artiq_red_cpmg_mbi_config import ARQICK_DoPulses_Red_CPMG_Mbi


class ARQICK_red_CPMGXY8_200ps(EnvExperiment, ARQICK_DoPulses_Red_CPMG_Mbi):
    def build(self):
        self.setattr_argument("n_cpmg", NumberValue(8, precision=0, step=1))
        self.setattr_argument("pi2_duration_tdds", NumberValue(95, precision=0, step=1))
        self.setattr_argument("tau_low_tdds", NumberValue(500, precision=0, step=1, min=500)) # theres a min delay here due to qick
        self.setattr_argument("tau_high_tdds", NumberValue(1000, precision=0, step=1))
        self.setattr_argument("tau_step_tdds", NumberValue(100, precision=0, step=1))
