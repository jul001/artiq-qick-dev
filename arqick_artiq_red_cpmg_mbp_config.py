...
class ARQICK_DoPulses_Red_CPMG_Mbp:
    kernel_invariants = {"data_size",
                         ...
                         "charge_check_mu",
                         "charge_check_laser1_mu",
                         "charge_check_laser2_mu",
                         "photon_threshold",
                         "wait_time_mu",
                         "wait_time_laser1_mu",
                         "wait_time_laser2_mu",
                         ...
                         "prep_nv_total_duration_mu",
                         "tot_time_for_ttl2_readout_mu"
                         }
    ...
    def build_config(self):
        ...
        self.setattr_argument("photon_threshold", NumberValue(3, precision=0, step=1))
    ...
    def prepare_config(self, Fineres: bool = False):
        ...
        self.pulse_width_ns = 50 * ns                                       # used to start the qick
        ...
        self.delay_to_align_second_ttl5_to_readout_window_ns = 95 * ns
        ...
        self.prep_nv_total_duration_us = self.read_to_red1 + self.green_init_duration + self.charge_check + self.delay_after_prep_nv
        self.prep_nv_total_duration_mu = self.core.seconds_to_mu(self.prep_nv_total_duration_us)
        self.tot_time_for_ttl2_readout_mu = self.core.seconds_to_mu(self.prep_nv_total_duration_us 
        ...
        self.tot_time_after_pmod_out_to_readout = (self.prep_nv_total_duration_us
                                                   + self.delay_to_align_second_ttl5_to_readout_window_ns)
                                                #    + self.delay_to_align_ttl5_to_readout_window_ns)
                                                #    + self.first_pmod_out_duration_ns)
                                                #    - self.pulse_width_ns
                                                #    - self.qick_adc_readout_to_pmod_out_delay_ns)
        ...
        self.ttl4_pulses_mu = self.core.seconds_to_mu(self.ttl4_pulses)
        ...
        self.charge_check_laser1_mu = self.core.seconds_to_mu(self.charge_check + self.laser1_pulse_width_correction)
        self.charge_check_laser2_mu = self.core.seconds_to_mu(self.charge_check + self.laser2_pulse_width_correction)
        self.charge_check_mu = self.core.seconds_to_mu(self.charge_check)
        ...
        self.set_dataset("on_random_counts", [], broadcast=False)  # to store the tau values for x axis
    ...
    def run_config(self):
        ...
        # self.random_counts = [5, 15, 3, 2, 1, 1]
        self.random_counts = np.random.randint(low=1, high=16, size=2)
        self.append_to_dataset("on_random_counts", self.random_counts)
        print(self.tau_list)
        print(self.tau_list2)
        print(len(self.tau_list2))
        print(self.random_counts)
        print(len(self.random_counts))
    ...
    def do_pulses(self, freq):
        ...
        for i in range(self.n_cycles):
            currCount = self.random_counts[0]
            counts = 0
            # counts = self.random_counts[0]                                 # perform charge check
            x = 0
            n_repeats = -1
            while counts < self.photon_threshold:         # > self.photon_threshold:
                self.prep_nv(currCount)
                with parallel:
            ...
            for j in range(self.data_size-1):
                ...
                currCount = self.random_counts[j + 1]
                counts = 0 # self.random_counts[j + 1]
                n_repeats = -1
    ...
    def prep_nv(self, currCount):
        ...
        delay_mu(self.charge_check_laser2_mu)
        ...
        delay_mu(self.green_init_duration_mu)
        for i in range(2):
            self.ttl4.pulse_mu(self.ttl4_pulses_mu)
            delay_mu(self.ttl4_pulses_mu)
        # self.ttl4.pulse_mu(self.green_init_duration_mu)

        at_mu(cursor)
        delay_mu(self.green_init_duration_mu)
        delay_mu(self.read_to_green_mu)         # readout sequence
        self.ttl0_counter.gate_rising_mu(self.charge_check_mu)
    ...
    def pulse_artiq(self, index, start):
        if start:
            ...
            delay_mu(self.after_first_pmod_out_to_ttl5_mu)          # 300ns + prep_nv + 125ns - 410ns
        ...
        # else:
            ...
            # delay_mu(self.tot_time_after_pmod_out_to_readout_mu)    # prep_nv + delay_to_align_second_ttl5_to_readout_window_ns)
        else:
            cursor = now_mu()
            at_mu(cursor)
            delay_mu(self.tot_time_after_pmod_out_to_readout_mu)        # prep_nv(21.535us) + 95ns
            self.ttl5.pulse_mu(self.ttl5_pulse_width_mu)                # 500 ns charge check passed -> send pulse to qick adc
            at_mu(cursor)
            delay_mu(self.tot_time_for_ttl2_readout_mu)                 # 21.44us
